...
ARG ftp_proxy=$http_proxy
ARG no_proxy=af01p-png.devtools.intel.com,af01p-png.devtools.intel.com,ubit-artifactory-or.intel.com,*.intel.com

FROM ubuntu:22.04

ADD https://af01p-png.devtools.intel.com/artifactory/hspe-edge-png-local/ubuntu/keys/adl-hirsute-public.gpg /etc/apt/trusted.gpg.d/
RUN chmod 644 /etc/apt/trusted.gpg.d/adl-hirsute-public.gpg
RUN echo "deb https://af01p-png.devtools.intel.com/artifactory/hspe-edge-png-local/ubuntu/jammy/jammy/0301-1633/ jammy main non-free multimedia internal" > /etc/apt/sources.list.d/intel-internal.list
RUN echo "deb [trusted=yes] https://ubit-artifactory-or.intel.com/artifactory/turtle-creek-debian-local impish universe"  >> /etc/apt/sources.list.d/intel-internal.list
# apt pinning
RUN echo "Package: *\\nPin: origin af01p-png.devtools.intel.com\\nPin-Priority: 1001" >> /etc/apt/preferences.d/priorities
# Proxy
RUN if test -n "$http_proxy" ; then echo "http_proxy=$http_proxy" >> /etc/environment; fi
RUN if test -n "$https_proxy" ; then echo "https_proxy=$https_proxy" >> /etc/environment; fi
RUN if test -n "$http_proxy" ; then echo "HTTP_PROXY=$http_proxy" >> /etc/environment; fi
RUN if test -n "$https_proxy" ; then echo "HTTPS_PROXY=$https_proxy" >> /etc/environment; fi
RUN if test -n "$ftp_proxy" ; then echo "ftp_proxy=$ftp_proxy" >> /etc/environment; fi
RUN if test -n "$ftp_proxy" ; then echo "FTP_PROXY=$ftp_proxy" >> /etc/environment; fi
RUN if test -n "$no_proxy" ; then echo "no_proxy=$no_proxy" >> /etc/environment; fi
RUN if test -n "$no_proxy" ; then echo "NO_PROXY=$no_proxy" >> /etc/environment; fi

RUN if test -n "$ftp_proxy" ; then echo "Acquire::ftp::Proxy \"$ftp_proxy\";" > /etc/apt/apt.conf.d/99proxy.conf; fi
RUN if test -n "$http_proxy" ; then echo "Acquire::http::Proxy \"$http_proxy\";" >> /etc/apt/apt.conf.d/99proxy.conf; fi
RUN if test -n "$https_proxy" ; then echo "Acquire::https::Proxy \"$https_proxy\";" >> /etc/apt/apt.conf.d/99proxy.conf; fi
RUN if test -n "$no_proxy"; then perl -e 'foreach(split(q{,},$ARGV[0])){printf(qq{Acquire::https::proxy::%s "DIRECT";\n},$_)}' $no_proxy >> /etc/apt/apt.conf.d/99proxy.conf; fi
...
#use this
#apt-get install build-essential cmake ninja-build libgtk-3-dev libpng-dev libjpeg8-dev libjpeg-dev libwebp-dev libtiff5-dev libopenexr-dev libopenblas-dev libx11-dev libssl-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev
...
RUN tar xzvf l_openvino_toolkit_ubuntu20_2022.2.0.7713.af16ea1d79a_x86_64.tgz
RUN mkdir -p /opt/intel/openvino
RUN mv l_openvino_toolkit_ubuntu20_2022.2.0.7713.af16ea1d79a_x86_64/* /opt/intel/openvino/
RUN rm l_openvino_toolkit_ubuntu20_2022.2.0.7713.af16ea1d79a_x86_64.*
...
RUN wget https://github.com/oneapi-src/level-zero/releases/download/v1.6.2/level-zero-devel_1.6.2+u18.04_amd64.deb
RUN dpkg -i level-zero*.deb
RUN rm *.deb
